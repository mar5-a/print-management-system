...
const envSchema = z.object({
  ...
  JOB_CLEANUP_ENABLED: z.enum(['true', 'false']).default('true'),
  JOB_CLEANUP_INTERVAL_MS: z.coerce.number().int().positive().default(5 * 60 * 1000),
...
export const config = {
  ...
  jobCleanup: {
    enabled: env.JOB_CLEANUP_ENABLED === 'true',
    intervalMs: env.JOB_CLEANUP_INTERVAL_MS,
  },
